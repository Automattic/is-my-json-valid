...
declare function createValidator<T extends ObjectProps, R extends keyof T> (schema: ObjectSchema<T, R>, options?: any): ((input: factory.JSONValue, options?: any) => input is { [K in keyof T]: (K extends R ? ExtractSchemaType<T[K]> : ExtractSchemaType<T[K]> | undefined) }) & { errors: factory.ValidationError[] }
declare function createValidator<T extends GenericSchema> (schema: T, options?: any): ((input: factory.JSONValue, options?: any) => input is ExtractSchemaType<T>) & { errors: factory.ValidationError[] }

declare function createFilter<T extends ObjectProps, R extends keyof T> (schema: ObjectSchema<T, R>, options?: any): ((input: factory.JSONValue, options?: any) => { [K in keyof T]: (K extends R ? ExtractSchemaType<T[K]> : ExtractSchemaType<T[K]> | undefined) })
declare function createFilter<T extends GenericSchema> (schema: T, options?: any): ((input: factory.JSONValue, options?: any) => ExtractSchemaType<T>)
